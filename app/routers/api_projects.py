...
@router.post("")
async def post_create_project(body: CreateProjectRequest):
    ...
    try:
        meta = create_project(body.name, body.base, body.target)
...
async def patch_project(project_id: str, body: UpdateProjectRequest):
    meta = load_metadata(project_id)
    if not meta:
        raise HTTPException(404, "Proyecto no encontrado")
    if body.name:
